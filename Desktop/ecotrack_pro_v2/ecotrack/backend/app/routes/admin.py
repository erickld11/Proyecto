...
def list_all_users(db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    for user in users:
        ...
        result.append(AdminUserOut(
            ...
            total_consumptions=len(consumptions), total_carbon_kg=round(total_carbon, 2),
        ))
    return result
...
def list_all_consumptions(db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    for c in consumptions:
        ...
        result.append({
            ...
            "month": c.month, "electricity_kwh": c.electricity_kwh,
            "gas_kwh": c.gas_kwh, "water_liters": c.water_liters,
            "transport_km": c.transport_km, "carbon_footprint_kg": c.carbon_footprint_kg,
        })
    return result
...
def global_stats(db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    return {
        "total_users": total_users, "total_consumptions": total_consumptions,
        "total_carbon_kg": round(total_carbon, 2), "total_ai_plans": total_plans,
...
def seed_demo_data(db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    created = 0
    for month in months:
        existing = db.query(Consumption).filter(Consumption.user_id == admin.id, Consumption.month == month).first()
        if existing:
            continue
        ...
        db.add(Consumption(user_id=admin.id, month=month, electricity_kwh=elec, gas_kwh=gas,
                           water_liters=water, transport_km=transport, carbon_footprint_kg=carbon,
                           notes="Datos de demostracion"))
        created += 1
    db.commit()
    return {"message": f"Se insertaron {created} meses de datos de demostracion", "months_created": created}
