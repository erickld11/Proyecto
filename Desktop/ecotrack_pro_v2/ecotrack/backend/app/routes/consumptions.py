...
def create_consumption(data: ConsumptionCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    existing = db.query(Consumption).filter(Consumption.user_id == current_user.id, Consumption.month == data.month).first()
    if existing:
...
def list_consumptions(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return db.query(Consumption).filter(Consumption.user_id == current_user.id).order_by(Consumption.month.desc()).all()
...
def get_breakdown(consumption_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    consumption = db.query(Consumption).filter(Consumption.id == consumption_id, Consumption.user_id == current_user.id).first()
    if not consumption:
...
def export_csv(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    import io
    consumptions = db.query(Consumption).filter(Consumption.user_id == current_user.id).order_by(Consumption.month.asc()).all()
    output = io.StringIO()
