...
def generate_action_plan(month, electricity_kwh, gas_kwh, water_liters, transport_km, carbon_kg, previous_carbon_kg=None):
    ...
    try:
        ...
        if previous_carbon_kg and previous_carbon_kg > 0:
            ...
            direction = "aumento" if change > 0 else "disminuyo"
...
{change_text}
Proporciona: 1) Analisis breve 2) 3 acciones prioritarias 3) Objetivo para el proximo mes. Usa emojis."""
...
def _generate_fallback_plan(month, electricity_kwh, gas_kwh, water_liters, transport_km, carbon_kg, previous_carbon_kg=None):
    ...
    if electricity_kwh > 500:
        tips.append("Electricidad elevada. Instala sensores de presencia y revisa climatizacion fuera del horario laboral.")
    else:
        tips.append("Buen control electrico. Sigue apagando equipos al finalizar la jornada.")
    if gas_kwh > 300:
        tips.append("Gas significativo. Revisa el aislamiento y ajusta el termostato a 21 grados. Cada grado menos ahorra un 7%.")
    else:
        tips.append("Uso de gas razonable. Mantener el mantenimiento regular de la calefaccion.")
    if water_liters > 5000:
        tips.append("Agua alta. Instala grifos de bajo consumo y revisa posibles fugas.")
    else:
        tips.append("Buen control del agua. Considera sistemas de reutilizacion.")
    if transport_km > 1000:
        tips.append("Alta movilidad. Fomenta teletrabajo 2 dias por semana y carpooling entre empleados.")
    else:
        tips.append("Buen uso del transporte. Continua fomentando reuniones virtuales.")
    change_info = ""
    if previous_carbon_kg and previous_carbon_kg > 0:
        change = ((carbon_kg - previous_carbon_kg) / previous_carbon_kg) * 100
        if change > 0:
            change_info = f"\nTu huella aumento un {change:.1f}% respecto al mes anterior."
        else:
            change_info = f"\nEnhorabuena! Tu huella bajo un {abs(change):.1f}% respecto al mes anterior."
    objective = max(0, carbon_kg * 0.9)
    return f"""PLAN DE ACCION PERSONALIZADO - {month}
{'='*50}
...
OBJETIVO PARA EL PROXIMO MES
Reducir la huella a {objective:.1f} kg CO2 (reduccion del 10%).
